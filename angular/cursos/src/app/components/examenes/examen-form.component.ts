...
export class ExamenFormComponent extends CommonFormComponent<Examen, ExamenService> implements OnInit {
    ...
    this.route.paramMap.subscribe(params => {
      ...
      if (id) {
        this.service.ver(id).subscribe(m => {
          ...
          this.cargarHijos();
          // this.service.findAllAsignaturas().subscribe(asignaturas => this.asignaturasHija = asignaturas.filter(a => a.padre && a.padre.id === this.model.asignaturaPadre.id));
        });
      }
    });

    this.service.findAllAsignaturas().subscribe(asignaturas => this.asignaturasPadre = asignaturas.filter(a => !a.padre));
  }

  public crear(): void {
    if (this.model.preguntas.length === 0) {
      // Swal.fire('Error Preguntas:', 'Exámen debe tener preguntas', 'error');
  ...
  eliminarPreguntasVacias(): void {
    this.model.preguntas = this.model.preguntas.filter(p => p.texto !== null && p.texto !== undefined && p.texto.length > 0);
  }
